...
public class CustomSeekBarPreference extends Preference implements SeekBar.OnSeekBarChangeListener,
    ...
    public CustomSeekBarPreference(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        try {
            ...
            Log.e(logTag, "Invalid interval value", e);
        }
    ...
    public void onBindViewHolder(PreferenceViewHolder holder) {
        ...
        mResetImageView.setContentDescription(getContext().getString(R.string.custom_seekbar_reset));
        mMinusImageView.setContentDescription(getContext().getString(R.string.custom_seekbar_decrease));
        mPlusImageView.setContentDescription(getContext().getString(R.string.custom_seekbar_increase));
    ...
    private String getDisplayedValueText() {
        final int shownValue = (!mTrackingTouch || mContinuousUpdates) ? mValue : mTrackingValue;
        final String displayedText = getTextValue(shownValue);

        if (!mTrackingTouch && mContinuousUpdates && mDefaultValueExists && mValue == mDefaultValue) {
            return displayedText + " (" + getContext().getString(R.string.custom_seekbar_default_value) + ")";
        }
    ...
    private int getCurrentA11yValue() {
        return (!mTrackingTouch || mContinuousUpdates) ? mValue : mTrackingValue;
    }

    protected void updateValueViews() {
        final String displayedValueText = getDisplayedValueText();

        if (mValueTextView != null) {
            mValueTextView.setText(getContext().getString(R.string.custom_seekbar_value, displayedValueText));
        }

        mSeekBar.setContentDescription(getContext().getString(R.string.custom_seekbar_current_value,
            getTextValue(getCurrentA11yValue())));
        ...
        updateButtonState(mMinusImageView, !(mValue == mMinValue || mTrackingTouch));
        updateButtonState(mPlusImageView, !(mValue == mMaxValue || mTrackingTouch));
    }
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.reset) {
            Toast.makeText(getContext(), getContext().getString(R.string.custom_seekbar_default_value_to_set,
