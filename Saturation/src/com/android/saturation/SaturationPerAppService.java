...
public class SaturationPerAppService extends Service implements SharedPreferences.OnSharedPreferenceChangeListener {
            ...
            try {
                ...
                if (focused != null && focused.topActivity != null) {
                    ...
                    if (!pkg.equals(mCurrentTopPkg)) {
                        Log.d(TAG, "App changed from " + mCurrentTopPkg + " to " + pkg);
                        mCurrentTopPkg = pkg;
                        applyForTopApp();
                    }
                } else {
                    Log.d(TAG, "No focused activity found");
                }
            } catch (Throwable t) {
    ...
    private void reloadPrefs() {
        ...
        Log.d(TAG, "Prefs reloaded - saturation: " + mSavedSeekBarValue + 
              ", perAppEnabled: " + mPerAppEnabled + 
              ", disabledPackages: " + mDisabledPackages);
    }
    ...
    private void applyForTopApp() {
        if (!mPerAppEnabled) {
            Log.d(TAG, "Per-app disabled, applying saved saturation");
            applySaved();
            return;
        }
        boolean enabledForApp = !mDisabledPackages.contains(mCurrentTopPkg);
        Log.d(TAG, "Per-app enabled, app: " + mCurrentTopPkg + ", enabled: " + enabledForApp);
        if (enabledForApp) {
            Log.d(TAG, "Applying saved saturation: " + mSavedSeekBarValue);
            applySaved();
        } else {
            Log.d(TAG, "Applying neutral saturation (disabled for this app)");
            applyNeutral();
        }
    }
    ...
    private void applySaturation(float saturation) {
        Log.d(TAG, "Applying saturation: " + saturation);
        ...
        if (surfaceFlinger != null) {
            try {
                ...
                data.recycle();
                Log.d(TAG, "Saturation applied successfully: " + saturation);
            } catch (RemoteException e) {
